...
if SERVER then
    ...
    local function IsValidVoteType(votetype)
        if not isnumber(votetype) then return false end
        if votetype <= VOTE_TYPE_INVALID or votetype > VOTE_TYPE_LAST then return false end
    ...
    function GM:StartVote(ply, votetype, time, params, options, excluded, resultFn)
        if not IsValidVoteType(votetype) then return false end
        -- We only support a single vote right now.
        if self:HasActiveVotes() == true then return end
        voteid = voteid + 1
    ...
    hook.Add("PlayerBindPress", "LambdaVoteInput", function(ply, bind, pressed)
        if activeVoteId == nil then return end
        if string.match(bind, "slot%d+") == nil then return end
        local vote = currentVotes[activeVoteId]
        if vote == nil then return end
        if vote.finished == true or vote.canVote == false then return end
