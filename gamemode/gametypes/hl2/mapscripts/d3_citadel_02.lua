...
function MAPSCRIPT:Think()
    ...
    for k, v in pairs(self.PlayerQueue) do
        if ct < v.timestamp then continue end
    ...
    if SERVER then
        ...
        ents.WaitForEntityByName("trigger_leave_level", function(ent)
            ...
            ent.OnTrigger = function()
                TriggerOutputs({{"trigger_vphysics_03_fall", "Kill", 0.0, ""}, {"cit02_cit03_trans", "ChangeLevel", 2.0, ""}})
            end
        end)
        ...
        dissolveTrigger:SetupTrigger(Vector(3889.122803, 13407.586914, 3520.031250), Angle(0, 0, 0), Vector(-160, -260, 0), Vector(160, 260, 30))
