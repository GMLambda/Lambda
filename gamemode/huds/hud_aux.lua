...
function PANEL:Init()
    ...
    self.Animations = {self.AnimateSizeChange, self.AnimateFadeIn, self.AnimateFadeOut}
    self.TextX = 8
    self.TextY = 15
    self.Text2X = 8
    self.Text2Y = 25
...
function PANEL:Paint(width, height)
    ...
    for i = enabledChunks, chunkCount do
        surface.DrawRect(xpos, ypos, chunkWidth, barHeight)
        xpos = xpos + (chunkWidth + util.ScreenScaleH(self.BarChunkGap))
    end
    ...
    if sizeChange == true then
        ...
        self.AnimateSizeChange:Start(1, {
            targetH = targetH,
            targetW = targetW,
            targetY = targetY,
            targetX = targetX
