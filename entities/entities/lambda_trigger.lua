...
if SERVER then
    ...
    function ENT:PreInitialize()
        ...
        self:SetupNWVar("Disabled", "bool", {
            ...
            OnChange = self.HandleDisableChange
        })

        self:SetupNWVar("WaitTime", "float", {
            Default = 0.2,
            KeyValue = "wait"
        })

        self:SetupNWVar("FilterName", "string", {
            Default = "",
            KeyValue = "filtername"
        })

        self:SetupNWVar("WaitForTeam", "bool", {
            Default = false,
            KeyValue = "teamwait"
        })

        self:SetupNWVar("LockPlayers", "bool", {
            Default = false,
            KeyValue = "lockplayers"
        })

        self:SetupNWVar("Blocked", "bool", {
            Default = false,
            KeyValue = "blocked",
            OnChange = self.HandleBlockingUpdate
        })

        -- If WaitForTeam is enabled this specifies the timeout to when to force trigger.
        self:SetupNWVar("Timeout", "float", {
            Default = 0,
            KeyValue = "timeout"
        })

        -- Teleport all players on timeout.
        self:SetupNWVar("TimeoutTeleport", "bool", {
            Default = true,
            KeyValue = "timeoutteleport"
        })

        self:SetupNWVar("DisableEndTouch", "bool", {
            Default = false,
            KeyValue = "disableendtouch"
        })

        self:SetupNWVar("ShowWait", "bool", {
            Default = true,
            KeyValue = "showwait"
        })

        self:SetupNWVar("MessagePos", "vector", {
    ...
    function ENT:Touch(ent)
        ...
        if self.NextWait ~= nil and self.NextWait ~= 0 then
            if CurTime() < self.NextWait then return end
            self.NextWait = nil
        end
    ...
    function ENT:PassesTriggerFilters(ent)
        if self:HasSpawnFlags(SF_TRIGGER_ALLOW_ALL) or (self:HasSpawnFlags(SF_TRIGGER_ALLOW_CLIENTS) and ent:IsPlayer()) or (self:HasSpawnFlags(SF_TRIGGER_ALLOW_NPCS) and ent:IsNPC()) or (self:HasSpawnFlags(SF_TRIGGER_ALLOW_PUSHABLES) and ent:GetClass() == "func_pushable") or (self:HasSpawnFlags(SF_TRIGGER_ALLOW_PHYSICS) and ent:GetMoveType() == MOVETYPE_VPHYSICS) then
            ...
            if self:HasSpawnFlags(SF_TRIGGER_ONLY_CLIENTS_IN_VEHICLES) and ent:IsPlayer() then
                if ent:InVehicle() == false then return false end --DbgPrint("Client must be in vehicle")
            end

            if self:HasSpawnFlags(SF_TRIGGER_ONLY_PLAYER_WITH_HEV) and ent:IsPlayer() then
                if ent:IsSuitEquipped() == false then return false end
            end

            if self:HasSpawnFlags(SF_TRIGGER_ONLY_CLIENTS_OUT_OF_VEHICLES) and ent:IsPlayer() then
                if ent:InVehicle() == true then return false end --DbgPrint("Client must be not in vehicle")
            end
